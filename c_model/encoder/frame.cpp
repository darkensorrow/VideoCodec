...
m_height(height)
{
	...
	{
		//std::cout << bytes.size() << std::endl << m_width << std::endl << height << std::endl;
...
void Frame::SlideWindowUpdate(
	int block_size,
	int r,
	int search_window_width,
	int search_window_height,
	int patch_block_width,
	int remove_block_width,
	bool remove_unused_blocks,
	Frame& SearchWindowFrame,
	std::vector<BYTE_T> ref_search_window_vect)
{
	std::vector<BYTE_T> SearchWindowVect;
	std::vector<BYTE_T> NewFrameConentVect;
	
	NewFrameConentVect = ref_search_window_vect;
	SearchWindowVect = SearchWindowFrame.y_values.as_vec();
	for (int i = 0; i < search_window_height; i++){
		if (remove_unused_blocks){
			SearchWindowVect.erase(SearchWindowVect.begin() + i*(search_window_width), SearchWindowVect.begin() + i*(search_window_width) + remove_block_width);
			if (i != search_window_height -1){
				SearchWindowVect.insert(SearchWindowVect.begin() + (i*search_window_width) + search_window_width - remove_block_width, NewFrameConentVect.begin(), NewFrameConentVect.begin() + patch_block_width);
			}
			else {
				SearchWindowVect.insert(SearchWindowVect.end(), NewFrameConentVect.begin(), NewFrameConentVect.begin() + patch_block_width);
			}
		}
		else {
			SearchWindowVect.insert(SearchWindowVect.begin() + search_window_width - block_size + i*(search_window_width), NewFrameConentVect.begin(), NewFrameConentVect.begin() + patch_block_width);
		}
		NewFrameConentVect.erase(NewFrameConentVect.begin(), NewFrameConentVect.begin() + patch_block_width);
	}
	SearchWindowFrame = Frame(SearchWindowVect, search_window_width, search_window_height, true);
}

ByteMatrix Frame::transpose(unsigned int width, unsigned int height)
{
	std::vector<BYTE_T> y_values_vec = y_values.as_vec();
	std::vector<BYTE_T> y_values_vec_transpose;
	for (unsigned int j = 0; j < height; j++)
	{
		for (unsigned int i = j; i < width*height; i += height)
		{
			y_values_vec_transpose.push_back(y_values_vec[i]);
		}
	}
	ByteMatrix ret(y_values_vec_transpose, width, height);
	return ret;
}
...
std::vector<BYTE_T> Frame::get_y_vect_at(COORD_T coord, unsigned int i) const
{
	//assert(y_values.block_coord_is_legal(coord, i, true));

	return y_values.get_vect_at(coord, i);
}
...
std::tuple<unsigned int, ByteMatrix, MV_T> PFrame::search_for_best_ref(
	...
	bool fast_me,
	const MV_T& last_mv,
	int& num_data_transfer,
	std::ofstream& data_vect_txt,
	bool dump_search_window_data_transfer, 
	bool slide_window_update)
{
	if (slide_window_update) {
		static unsigned int ReqVld, ReqAddr, ReqLen, RdRspVld;
		std::vector<BYTE_T> RdRspData;

		ReqLen = bus_width;

		int FrameWidth = ref_frames[0].get_width();
		int FrameHeight = ref_frames[0].get_height();

		//ref_vect.print(blk_txt);

		//send cur_block
		SearchQ search_block_vectors;
		COORD_T block_vect_coord;
		std::vector<BYTE_T> rcvd_block_vect;
		int search_block_i, search_block_j;
		ReqAddr = 0x1000;
		for (unsigned int i = 0; i < block_size; i++)
		{
			for (unsigned int j = 0; j < block_size; j += ReqLen)
			{
				block_vect_coord.first = i;
				block_vect_coord.second = j;
				search_block_vectors.push(std::make_pair(i, j));
			}
		}

		while (!search_block_vectors.empty())
		{
			std::tie(search_block_i, search_block_j) = search_block_vectors.pop();
			block_vect_coord.first = search_block_i;
			block_vect_coord.second = search_block_j;
			ReqVld = 1;
			if (ReqVld){
				std::tie(RdRspVld, RdRspData) = transfer_data(ReqAddr, ReqLen, block_vect_coord, cur_block, ref_frames, r, block_size, last_mv);
				ReqVld = 0;
				num_data_transfer++;
				if (dump_search_window_data_transfer){
					for (auto& elem : RdRspData)
					{
						data_vect_txt << std::setw(3) << static_cast<unsigned>(elem) << " ";
					}
					data_vect_txt << std::endl;
				}
			}
			if (RdRspVld){
				rcvd_block_vect.insert(rcvd_block_vect.end(), RdRspData.begin(), RdRspData.end());
				RdRspVld = 0;
			}
		}
		ByteMatrix rcvd_cur_block(rcvd_block_vect, block_size, block_size);

		//set up search window //
		SearchQ search_window_vectors;
		COORD_T ref_vect_coord;
		std::vector<BYTE_T> ref_search_window_vect, ref_search_window_vect_transpose;
		static int search_window_i, search_window_j, ref_vect_x, ref_vect_y, search_window_width, search_window_height;
		ReqAddr = 0x0;
		bool update_search_window = false;

		if (int(cur_coord.second) == 0){
			//set whole window search
			if (int(cur_coord.first) - r < 0){
				ref_vect_x = 0;
				ref_vect_y = 0;
				search_window_width = block_size + r;
				search_window_height = int(cur_coord.first) + block_size + r;
				update_search_window = true;
			}
			else if (int(cur_coord.first) + block_size + r > FrameHeight) {
				ref_vect_x = 0;
				ref_vect_y = int(cur_coord.first) - r;
				search_window_width = block_size + r;
				search_window_height = FrameHeight - int(cur_coord.first) + r;
				update_search_window = true;
			}
			//else if (int(cur_coord.first) + r >= FrameHeight) {
				//ref_vect_x = 0;
				//ref_vect_y = FrameHeight - 2*r - block_size;
				//need to handel padding
			//}
			else {
				ref_vect_x = 0;
				ref_vect_y = int(cur_coord.first) - r;
				search_window_width = block_size + r;
				search_window_height = block_size + 2 * r;
				update_search_window = true;
			}
			if (update_search_window){
				for (search_window_i = 0; search_window_i < search_window_width; search_window_i++)
				{
					for (search_window_j = 0; search_window_j < search_window_height; search_window_j += bus_width)
					{
						search_window_vectors.push(std::make_pair(search_window_i, search_window_j));
					}
				}

				while (!search_window_vectors.empty())
				{
					std::tie(search_window_i, search_window_j) = search_window_vectors.pop();
					ref_vect_coord.first = ref_vect_x + search_window_i;
					ref_vect_coord.second = ref_vect_y + search_window_j;
					ReqVld = 1;
					if (ReqVld){
						std::tie(RdRspVld, RdRspData) = transfer_data(ReqAddr, ReqLen, ref_vect_coord, cur_block, ref_frames, r, block_size, last_mv);
						ReqVld = 0;
						num_data_transfer++;
						if (dump_search_window_data_transfer){
							for (auto& elem : RdRspData)
					...
					if (RdRspVld){
						ref_search_window_vect.insert(ref_search_window_vect.end(), RdRspData.begin(), RdRspData.end());
						RdRspVld = 0;
					}
				}

				for (unsigned int j = 0; j < search_window_height; j++)
				{
					for (unsigned int i = j; i < search_window_height*search_window_width; i += search_window_height)
					{
						ref_search_window_vect_transpose.push_back(ref_search_window_vect[i]);
					}
				}
				//std::cout << int(cur_coord.second) << std::endl << int(cur_coord.first) << std::endl;
				SearchWindowFrame = Frame(ref_search_window_vect_transpose, search_window_width, search_window_height, true);
				ref_search_window_vect_transpose.clear();
			}
		}
		else {
			//update sliding window
			//if (int(cur_coord.second) - r < 0) {
				//ref_vect_x = int(cur_coord.second) + block_size + r;
				//ref_vect_y = int(cur_coord.first);
				//search_window_width = int(cur_coord.second) + block_size + r;
				//search_window_height = block_size + r;
				//update_search_window = true;
			//}
			//else if (int(cur_coord.second) + block_size > FrameWidth) {
				//need to handel padding
			//}
			static bool remove_unused_blocks;
			static int remove_block_width;
			static int patch_block_width;
			update_search_window = false;
			if (int(cur_coord.second) + block_size + r <= FrameWidth) {
				ref_vect_x = int(cur_coord.second) + r;
				//ref_vect_y = int(cur_coord.first);
				
				//search_window_height = FrameHeight - int(cur_coord.first) + r;
				update_search_window = true;
				patch_block_width = block_size;
				if (int(cur_coord.second) - r <= 0) {
					search_window_width = int(cur_coord.second) + r + block_size;
					remove_unused_blocks = false;
				}
				else {
					search_window_width = 2 * r + block_size;
					remove_unused_blocks = true;
					if (int(cur_coord.second) < r + block_size){
						remove_block_width = int(cur_coord.second) - r;
					}
					else {
						remove_block_width = block_size;
					}
				}
			}
			else if (int(cur_coord.second) + block_size + r > FrameWidth && int(cur_coord.second) + r < FrameWidth){
				ref_vect_x = int(cur_coord.second) + r;
				update_search_window = true;
				patch_block_width = FrameWidth - int(cur_coord.second) - r;
				search_window_width = FrameWidth - int(cur_coord.second) + r;
				remove_unused_blocks = true;
				remove_block_width = block_size;
			}
			if (update_search_window) {
				
				for (search_window_i = 0; search_window_i < patch_block_width; search_window_i++)
				{
					for (search_window_j = 0; search_window_j < search_window_height; search_window_j += bus_width)
					{
						search_window_vectors.push(std::make_pair(search_window_i, search_window_j));
				...
				//std::cout << int(cur_coord.second) << std::endl << int(cur_coord.first) << std::endl;
				SearchWindowFrame.SlideWindowUpdate(block_size, r, search_window_width, search_window_height, patch_block_width, remove_block_width, remove_unused_blocks, SearchWindowFrame, ref_search_window_vect);
			}
		}
...
std::tuple<unsigned int, std::vector<BYTE_T>> PFrame::transfer_data(
	const unsigned int ReqAddr,
	const unsigned int ReqLen,
	const COORD_T& ref_vect_coord,
	const ByteMatrix& cur_block,
	const std::deque<Frame>& ref_frames,
	int r, 
	unsigned int block_size,
	const MV_T& last_mv)
{
	std::vector<BYTE_T> trans_data_vect;
	if (ReqAddr < 0x1000) {
		//std::cout << ref_vect_coord.first << " " << ref_vect_coord.second << std::endl;
		//search window
		trans_data_vect = ref_frames[0].get_y_vect_at(ref_vect_coord, ReqLen);
	}
	else {	
		//std::cout << ref_vect_coord.first << " " << ref_vect_coord.second << std::endl;
		//cur block
		trans_data_vect = cur_block.get_vect_at(ref_vect_coord, ReqLen);
	}
	unsigned int RdRspVld = 1;
	return std::make_tuple(RdRspVld, trans_data_vect);
}


PFrame::PFrame(const Frame& cur_frame, const std::deque<Frame>& ref_frames, Frame& SearchWindowFrame, unsigned int i, int r, int bus_width, unsigned int qp, double &ME_TIME, int &num_data_transfer, std::ofstream &data_vect_txt, bool dump_search_window_data_transfer, bool slide_window_update)
: m_block_size(i), m_frame_width(cur_frame.get_width()), m_frame_height(cur_frame.get_height())
{
	...
	for(auto& cur_block_t : cur_frame.get_y_block_vec(m_block_size))
	{
		...
		MV_T full_res_mv;
		clock_t ME_begin = std::clock();
		std::tie(min_full_cost, best_full_ref_block, full_res_mv) = PFrame::search_for_best_ref(cur_coord, cur_block, ref_frames, SearchWindowFrame, r, bus_width, m_block_size, qp, fast_me, last_mv, num_data_transfer, data_vect_txt, dump_search_window_data_transfer, slide_window_update);
		clock_t ME_end = std::clock();
		ME_TIME+=double(ME_end - ME_begin) / CLOCKS_PER_SEC;
		if(vbs_enable && m_block_size > 2 && m_block_size % 2 == 0)
		{
			...
			std::tie(min_top_left_cost, best_top_left_ref, top_left_res_mv) = PFrame::search_for_best_ref(cur_coord, top_left_block, ref_frames, SearchWindowFrame, r, bus_width, m_block_size / 2, (qp>0) ? qp - 1 : 0, fast_me, last_mv, num_data_transfer, data_vect_txt, dump_search_window_data_transfer, slide_window_update);
			...
			std::tie(min_top_right_cost, best_top_right_ref, top_right_res_mv) = PFrame::search_for_best_ref(cur_coord, top_right_block, ref_frames, SearchWindowFrame, r, bus_width, m_block_size / 2, (qp>0) ? qp - 1 : 0, fast_me, last_mv, num_data_transfer, data_vect_txt, dump_search_window_data_transfer, slide_window_update);
			...
			std::tie(min_bot_left_cost, best_bot_left_ref, bot_left_res_mv) = PFrame::search_for_best_ref(cur_coord, bot_left_block, ref_frames, SearchWindowFrame, r, bus_width, m_block_size / 2, (qp>0) ? qp - 1 : 0, fast_me, last_mv, num_data_transfer, data_vect_txt, dump_search_window_data_transfer, slide_window_update);
			...
			std::tie(min_bot_right_cost, best_bot_right_ref, bot_right_res_mv) = PFrame::search_for_best_ref(cur_coord, bot_right_block, ref_frames, SearchWindowFrame, r, bus_width, m_block_size / 2, (qp>0) ? qp - 1 : 0, fast_me, last_mv, num_data_transfer, data_vect_txt, dump_search_window_data_transfer, slide_window_update);
...
void PFrame::print_mv(std::ostream& mv_dist_out)
{
	assert(m_frame_width % m_block_size == 0);
	...
	for(auto& mv_and_res_block: m_mv_and_residuals)
	{
		int dist = ceil(sqrt(pow(mv_and_res_block.first.x, 2) + pow(mv_and_res_block.first.y, 2)));
		mv_dist_out << std::setw(6) <<dist;

		iblock++;
		if (iblock == blocks_wide)
		{
			mv_dist_out << std::endl;
			iblock = 0;
		}
	}
	mv_dist_out << std::endl;
