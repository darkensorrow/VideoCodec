...
std::vector<BYTE_T> ByteMatrix::get_vect_at(COORD_T coord, unsigned int i) const
{
	//assert(block_coord_is_legal(coord, i, true));

	std::vector<BYTE_T> vec;
	unsigned int col = coord.first; 
	for (unsigned int row = coord.second; row < coord.second + i; row++)
	{
		vec.push_back(m_matrix[row][col]);
	}
