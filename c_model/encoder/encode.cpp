...
int main(int argc, char* argv[])
{
	...
	unsigned int block_size, search_range, bus_width, qp;
	...
	CFG_LOAD_OPT_MANDATORY("bus_width", bus_width, all_values_loaded)
	...
	CFG_LOAD_OPT_DEFAULT("dump_search_window_data_transfer", dump_search_window_data_transfer, false);
	
	bool slide_window_update;
	CFG_LOAD_OPT_DEFAULT("slide_window_update", slide_window_update, false);
	...
	std::ofstream real_outfile, ref_outfile, res_outfile, mvs_txt, res_txt, mv_dist_txt, data_vect_txt;
	if(dump_debug_files)
	{
		...
		mv_dist_txt = std::ofstream ("mv_dist.txt");
	}
	
	data_vect_txt = std::ofstream("data_vect.txt");
	...
	std::cout << std::setw(12) << "SAD" << std::setw(12) << "PSNR" << std::setw(12) << "SSIM" << std::setw(12) << "Bytes" << std::setw(20) << "Data_Transfer_Bytes" << std::setw(12) << "ME Time" << std::setw(12) << "Time";
	std::cout << std::endl;
	...
	{	
		...
		{
			PFrame pf(cur_frame, ref_frames, SearchWindowFrame, block_size, search_range, bus_width, qp, ME_TIME, num_data_transfer, data_vect_txt, dump_search_window_data_transfer, slide_window_update);
			...
			if(dump_debug_files)
			{
				mvs_txt << "Frame " << iframe << ":";
				res_txt << "Frame " << iframe << ":";
				mv_dist_txt << "Frame " << iframe << ":"<<std::endl;
				pf.print(mvs_txt, res_txt);
				pf.print_mv(mv_dist_txt);
			}
		...
		std::cout << std::setw(20) << num_data_transfer * bus_width;
		std::cout << std::setw(12) << ME_TIME;
