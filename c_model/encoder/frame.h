...
extern double ME_TIME;
extern int num_data_transfer;
...
public:
	...
	std::vector<BYTE_T> get_y_vect_at(COORD_T coord, unsigned int i) const;
	...
	ByteMatrix transpose(unsigned int width, unsigned int height);
...
public:
	...
	PFrame(const Frame& cur_frame, const std::deque<Frame>& ref_frames, Frame& SearchWindowFrame, unsigned int i, int r, int bus_width, unsigned int qp, double &ME_TIME, int &num_data_transfer, std::ofstream& data_vect_txt, bool dump_search_window_data_transfer, bool slide_window_update);
	...
	void print_mv(std::ostream& mv_dist_out);
	...
	static std::tuple<unsigned int, std::vector<BYTE_T>> transfer_data(
